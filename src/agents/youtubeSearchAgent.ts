...
import { basicYoutubeSearchResponsePrompt,basicYoutubeSearchRetrieverPrompt } from "./Prompts/Prompts.js";
...
const createBasicYoutubeSearchAnsweringChain = (
  ...
  return RunnableSequence.from([
    RunnableMap.from({
      query: (input: YoutubeChainInput) => input.query,

      chat_history: (input: YoutubeChainInput) => input.chat_history,
  ...
  return RunnableSequence.from([
    ...
    RunnableLambda.from(async (input: string) => {
      ...
      const documents = res.results.map(
        (result) =>
          new Document({
            pageContent: result.content && result.content.length > 0
              ? result.content
              : result.title,
            metadata: {
              title: result.title,
...
const basicYoutubeSearch = (
  ...
  try {
    const chain = createBasicYoutubeSearchAnsweringChain(llm, embeddings);
